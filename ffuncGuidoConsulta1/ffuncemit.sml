...
fun emitTuplaRet lt =
	let	fun cellstp [] = 0
		| cellstp(TString::t) = 2 + cellstp t
		| cellstp((TT lt)::t) = cellstp lt + cellstp t
		| cellstp(_::t) = 1 + cellstp t
		val mc = cellstp lt
		fun et [] _ cells =
			"drop\n"^(if cells<mc then tS cells^" roll\n" else "")
		| et(TString::t) cnt cells =
			" dup "^tS cnt^" cells + @ 2@ rot ( string )\n"^et t (cnt-1) (cells+2)
		| et((TT lt)::t) cnt cells =
			"dup "^tS cnt^" cells + @ ( tupla )\n"^
			(et (rev lt) (length lt-1) 0)^
			et t (cnt-1) (cells + cellstp lt)
		| et(_::t) cnt cells =
			" dup "^tS cnt^" cells + @ swap ( int )\n"^et t (cnt-1) (cells+1)
	in	et lt (length lt-1) 0 end
...
let
	fun et [] _ 1 = " drop\n"
	| et [] _ 2 = " drop\n"
	| et [] _ cells = " drop "^tS(cells-1)^" roll \n"
	| et (TString::t) cnt cells = "dup "^tS cnt^" cells + @ 2@ rot\n"^et t (cnt+1) (cells+2)
	| et ((TT lt)::t) cnt cells = ("dup "^tS cnt ^" cells + @ \\ tupla\n "^et lt 0 cells)^et t (cnt+1) cells
	| et (_::t) cnt cells = "dup "^tS cnt ^" cells + @ swap "^et t (cnt+1) (cells+1)
in et (rev lt) 0 0 end
*)
...
fun roll1 0 = ""
| roll1 1 = " 2 roll over "
| roll1 n = tS (n+1) ^" roll swap "

fun roll2 0 = ""
| roll2 1 = "swap _mkString"
| roll2 n = tS (n+1) ^" roll "^tS (n+1)^" roll _mkString swap dup -rot" 

(* 0 cells + ! here 3 cells allot 3 roll 3 roll _mkString swap dup -rot  *)
fun despliega (TInt::t) cnt prof =
	roll1 prof ^" "^tS cnt^" cells + ! "^ despliega t (cnt+1) prof
| despliega (TString::t) cnt prof = 
	roll2 (prof+1) ^" "^ tS cnt ^" cells + ! " ^despliega t (cnt+1) prof
...
fun emitCall f param tret dp =
	...
	(case tret of
	TString => " _mkstring"
	(* ORI | TT tl => " "^genmktp (rev tl) *)
	| TT tl => " here "^ tS (length tl) ^" cells allot dup -rot \n"^despliega (rev tl) 0 0
	| _ => "")^"\n"
...
fun emitFunc f (arg:(string*Tipo)) tr body =
	":noname "^"{ _h _sl _"^(#1 arg)^" }\n"^ body^"\n"^
		(* ORI (case tr of TT lt => emitTuplaRet (rev lt) (length lt -1) | _ => "")^*)
		(case tr of TT lt => emitTuplaRet (rev lt)  | _ => "")^
		"\n_h here - allot\n;\nis _"^f^"\n"
